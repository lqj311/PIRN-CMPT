...
class HallucinationRGBFeatureToXYZInputMLP(nn.Module):
    def __init__(self, args, rgb_dim):
        super().__init__()
        self.args = args
        self.output_size = getattr(args, 'xyz_size', 224)
    ...
    def hallucination_generation(self, x):
        ...
        x = nn.functional.interpolate(x, size=(self.output_size, self.output_size), mode='bicubic')
        return x

    def forward(self, rgb_feature, xyz):
        # frgb B 3136 768 xyz B 3 H W xyz_hallucination B 3136 3/1
...
class HallucinationFeatureToInputConv(nn.Module):
    def __init__(self, args=None, dim=768):
        super().__init__()
        self.args = args
        self.output_size = self._resolve_output_size(args)
    ...
    @staticmethod
    def _resolve_output_size(args):
        if args is None:
            return 224

        train_method = getattr(args, 'train_method', '')
        main_modality = getattr(args, 'main_modality', '')
        if train_method == 'XYZFeatureToRGBInputConv' or main_modality == 'xyz':
            return getattr(args, 'rgb_size', 518)
        if train_method == 'RGBFeatureToXYZInputConv' or main_modality == 'rgb':
            return getattr(args, 'xyz_size', 224)
        return getattr(args, 'input_size', 224)

    def hallucination_generation(self, feature):
        feature = feature.transpose(1, 2)
        feature = feature.reshape(feature.shape[0], feature.shape[1], 56, 56)
        hallucination = self.conv1(feature)
        hallucination = nn.functional.interpolate(hallucination, size=(self.output_size, self.output_size), mode='bicubic')
    ...
    def forward(self, feature, img):
        # feature B 3136 768 img B 3 H W hallucination B 3 H W
        hallucination = self.hallucination_generation(feature)
        assert hallucination.shape[1:] == img.shape[1:]
