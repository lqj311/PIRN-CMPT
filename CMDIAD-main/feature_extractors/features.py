...
class Features(torch.nn.Module):
    def __init__(self, args, image_size=224, f_coreset=0.1, coreset_eps=0.9):
        ...
        if not getattr(args, 'skip_deep_feature_extractor', False):
            self.deep_feature_extractor = Model(
                device=self.device,
                rgb_backbone_name=args.rgb_backbone_name,
                checkpoint_path=args.rgb_checkpoint_path,
        ...
        self.rgb_feature_size = None
        self.rgb_feature_for_fusion_size = 56
    ...
    def get_rgb_patch(self, rgb_feature_maps):
        rgb_patch = torch.cat(rgb_feature_maps, 1)  # B C H W
        self.rgb_feature_size = tuple(rgb_patch.shape[-2:])
        rgb_patch2 = self.resize56(rgb_patch).squeeze(0)  # T768 56 56
        rgb_patch2 = rgb_patch2.reshape(rgb_patch2.shape[0], -1).T  # 3136 768
        rgb_patch = rgb_patch.squeeze(0).reshape(rgb_patch.shape[1], -1).T  # N T768
    ...
    def calculate_dist(self, single_patch, patch_lib):
        ...
        #     dist = torch.cdist(single_patch, patch_lib, p=1)
            ...
            if cp is None or distance is None:
                return torch.cdist(single_patch, patch_lib, p=1)
            single_patch_gpu = cp.asarray(single_patch)
            patch_lib_gpu = cp.asarray(patch_lib)
            dist = distance.cdist(single_patch_gpu, patch_lib_gpu, 'minkowski', p=1.).toDlpack()
            dist = torch.utils.dlpack.from_dlpack(dist).cpu()
        elif self.args.dist_method_s == 'cos_dist':
            if cp is None or distance is None:
                single_patch = torch.nn.functional.normalize(single_patch, dim=1)
                patch_lib = torch.nn.functional.normalize(patch_lib, dim=1)
                return 1 - single_patch @ patch_lib.t()
            single_patch_gpu = cp.asarray(single_patch)
            patch_lib_gpu = cp.asarray(patch_lib)
            dist = distance.cdist(single_patch_gpu, patch_lib_gpu, 'cosine').toDlpack()
            dist = torch.utils.dlpack.from_dlpack(dist).cpu()
        else:
